...
import {  EditListingCategory } from 'apis/actor';
...
import { createActorSchema } from '../../../helpers/validation';
...
export const EditActor = ()=>{
   ...
   const handleEditActor = async ()=>{
    await EditListingCategory(value.id, value.name,value.description);
    ...
    return <Container>
          ...
          onChange1={(e:any)=> setValue({ ...value, name: e.target.value })} requiredIcon name="name" label="Name" control={control} placeholder="Enter your name" />
            ...
            <FormControl variant="standard" sx={{width: "100%", marginTop:"10px"}}>
                ...
                <CreateFilmInput defaultValue={value.description}
                onChange1={(e:any)=> setValue({ ...value, description: e.target.value })} requiredIcon name="description" label="Description" control={control} placeholder="Enter Description " />
            </FormControl>
            
            <Button variant="contained" disabled={!isValid} onClick={handleSubmit(handleEditActor)} sx={{marginTop: "10px"}}>Edit</Button>              
            </MainWrapper>
           <CloseIcon>
