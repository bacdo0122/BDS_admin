...
export const EditNewsCategory = () => {
  ...
  const {
    ...
    defaultValues: {
      ...
      description:detail && detail.content,
    },
    resolver: yupResolver(createBannerSchema),
  });
  const handleEditBanner = async () => {
    await EditExisNewsCategory(value.id, value.name, value.description);
      ...
      <MainWrapper>
        ...
        <FormControl variant="standard" sx={{ width: '100%', marginTop: '10px' }}>
          ...
          <CreateFilmInput
            ...
            onChange1={(e: any) => setValue({ ...value, description: e.target.value })}
            requiredIcon
            name="description"
            label="description"
            control={control}
